...
Try {
	...
	[string]$appVendor = 'DensityDesign'
	[string]$appName = 'RAW Graphs'
	[string]$appVersion = ''
	[string]$appArch = ''
	[string]$appLang = 'EN'
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		## Show Welcome Message, close browsers, verify there is enough disk space to complete the install, and persist the prompt
		...
		Execute-Process -Path "SETX" -Parameters "/M PATH `"%PATH%;C:\Python34`"" -WindowStyle "Hidden" -PassThru
		...
		Copy-File -Path "$dirFiles\*" -Destination "$envSystemDrive\RAW" -Recurse
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Remove-File -Path "$envSystemDrive\RAW\*" -Recurse
		Remove-Folder -Path "$envSystemDrive\RAW"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA6/sSgtrisat7g
# OCSEAKDLjZnnk5XqqYGo+nngkbd6UaCCD4cwggQUMIIC/KADAgECAgsEAAAAAAEv
...
# gAAwGQYJKoZIhvcNAQkDMQwGCisGAQQBgjcCAQQwLwYJKoZIhvcNAQkEMSIEIIsV
# RA4q3ptXXU/noUISS3y6pTtNLlpmhY2c8NcwB4LvMA0GCSqGSIb3DQEBAQUABIIB
# AItJuMbGtpiNC/5P+MZGWgVUiJ66pJEVPdck2F5KsJxTkk/vDJ4ZvgMXMejZ4aUJ
# fpOQKpPn6NKLqgs4u+HGlhsplUW0fV7yCutGZqdskCdk/SAQrcBMbQa8PVN7hvRX
# gWX8UYxg3mSImRPU0yu/ZFQa6XcBsSIAOH0KQvfteubWAvMX9enw6pzVEw+dvwQA
# wWN1oGBRi/czu0cUFrivIZ6g6XpiEHMM0Bno52YXNvhUQnlvn5T49nvmEreScle8
# No1oh8gf4VWJH0Zh/v/kj+YPR4z1s6bTQwhVcFP1atJojOWXww9ZlZPDaKWauNBZ
# mfHtGqLQdWvfECU+HFZGafyhggKiMIICngYJKoZIhvcNAQkGMYICjzCCAosCAQEw
...
# ATAcBgkqhkiG9w0BCQUxDxcNMTcwMjA3MTc1NTE3WjAjBgkqhkiG9w0BCQQxFgQU
# TUeFXHxP3qw6WbyZ9oyRbQZ2sGUwgZ0GCyqGSIb3DQEJEAIMMYGNMIGKMIGHMIGE
...
# AQUABIIBAAy+7otZbHcmhxhTHhsKFZqi5YZMJFCseqm5s/2JklnnF3uctFaEfBvF
# fk+7o5BWvQrkrgVv/JyAVOtxUfaNDSYW/6ytlOLbYr6iIWYx9ZCk8nh9QmUVgjEI
# +qZelV5W+VcMBkVW9hSE53x1J8x+6s+lcu+6fzHbpClgb91/h+4cdR6AtAgyqij3
# NIx9QLVLeq4TYT+UVV3Yf0j/NsrpHApwWeR1mLyc8RQEFSjtYhbMr25MRtGwBXm+
# s/4IShWukbC6G2x8WbAbew3ULINU0TqWuY9ZzA8/UUohPBjZ6qd47+/OmAthw9pw
# 5LPgCLUl9BHjOaY8ASPZXsooXJkEPR8=
